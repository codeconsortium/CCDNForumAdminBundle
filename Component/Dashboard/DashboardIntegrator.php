    ...
    public function build(BuilderInterface $builder)
    {
        $builder
            ->addCategory('forum_admin')
                ->setLabel('dashboard.categories.forum_admin', array(), 'CCDNForumAdminBundle')
                ->addPages()
                    ->addPage('admin')
                        ->setLabel('dashboard.pages.admin', array(), 'CCDNForumAdminBundle')
                    ->end()
                    ->addPage('forum')
                        ->setLabel('dashboard.pages.forum', array(), 'CCDNForumAdminBundle')
                    ->end()
                ->end()
                ->addLinks()
                    ->addLink('edit_categories')
                        ...
                        ->setLabel('title.manage_boards', array(), 'CCDNForumAdminBundle')
                    ->end()
                    ->addLink('deleted_topics')
                        ...
                        ->setLabel('title.topic.show_deleted', array(), 'CCDNForumAdminBundle')
                    ->end()
                    ->addLink('deleted_posts')
                        ->setAuthRole('ROLE_ADMIN')
                        ->setRoute('ccdn_forum_admin_post_deleted_show_all')
                        ->setIcon('/bundles/ccdncomponentcommon/images/icons/Black/32x32/32x32_discussion.png')
                        ->setLabel('title.post.show_deleted', array(), 'CCDNForumAdminBundle')
                    ->end()
                    ->addLink('closed_topics')
                        ...
                        ->setLabel('title.topic.show_closed', array(), 'CCDNForumAdminBundle')
                    ->end()
                    ->addLink('locked_posts')
                        ->setAuthRole('ROLE_MODERATOR')
                        ->setRoute('ccdn_forum_admin_post_locked_show_all')
                        ->setIcon('/bundles/ccdncomponentcommon/images/icons/Black/32x32/32x32_lock.png')
                        ->setLabel('title.post.show_locked', array(), 'CCDNForumAdminBundle')
                    ->end()
                ->end()
            ->end()
        ;
